...
class _OrdenesTabState extends State<OrdenesTab> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      if (_workspaceId != null) {
        ...
        if (mounted) {
          setState(() {
            _almacenes = (results[0] as List<Almacen>).where((a) => (a.registros ?? 0) > 0).toList();
  ...
  Future<void> _submitOrder() async {
    if (_descController.text.isEmpty || _selectedArticulo == null || _selectedTipo == null) {
      AppToast.show(context, 'Por favor completa todos los campos requeridos', isError: true);
      return;
    }
    ...
    try {
      ...
      if (_selectedPhoto != null && _userId != null) {
        finalPhotoUrl = await _ordenesController.uploadPhotoOnly(_userId!, _selectedPhoto!.path);
      }
      ...
      final success = await _ordenesController.post(_ordenesController.endpoint, payload);

      if (success) {
        if (mounted) AppToast.show(context, 'Orden de servicio creada con éxito');
        _resetForm();
      } else {
        if (mounted) AppToast.show(context, 'Error al crear la orden', isError: true);
      }
    } catch (e) {
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'EN_PROGRESO': return AppColors.secondaryBase;
  ...
  Widget build(BuildContext context) {
    if (_isLoading && _almacenes.isEmpty) {
      return const Scaffold(body: Center(child: CircularProgressIndicator(color: AppColors.secondaryBase)));
    }
          ...
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                   const Text('Nueva Orden', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: AppColors.textBase)),
                   IconButton(
                    icon: const Icon(Icons.history_rounded, color: AppColors.secondaryBase),
                    onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const OrdenesHistoryView())),
                  ),
                ],
              ),
              ...
              Container(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(18), borderSide: const BorderSide(color: AppColors.secondaryBase, width: 2)),
                  ),
                ),
              ),
              ...
              Center(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    style: ElevatedButton.styleFrom(backgroundColor: AppColors.secondaryBase, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(25)), elevation: 2),
                    child: _isLoading 
                      ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                      : const Text('CREAR ORDEN', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.white)),
                  ),
                ),
              ),
              const SizedBox(height: 40),
        ...
        child: _selectedPhoto == null
          ? Center(
              child: Column(
                ...
                children: [
                  Icon(Icons.add_a_photo_outlined, size: 40, color: AppColors.secondaryBase.withOpacity(0.5)),
                  const SizedBox(height: 12),
                  const Text('Adjuntar evidencia', style: TextStyle(color: Colors.grey, fontWeight: FontWeight.w500)),
                ],
              ),
            )
          : ClipRRect(
  ...
  Widget _buildSelectionCard({required String title, required String subtitle, required IconData icon, String? imageUrl, VoidCallback? onTap, bool isSelected = false, bool enabled = true, Color? customColor}) {
              ...
              Container(
                ...
                child: ClipRRect(
                  ...
                  child: (imageUrl != null && imageUrl.isNotEmpty)
                    ? Image.network(imageUrl, fit: BoxFit.cover, errorBuilder: (_,__,___) => Icon(icon, color: color, size: 24))
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(subtitle, style: TextStyle(fontSize: 16, color: enabled ? AppColors.textBase : Colors.grey.shade400, fontWeight: FontWeight.bold, overflow: TextOverflow.ellipsis)),
                  ],
                ),
              ),
  ...
  void _showAlmacenSelection() {
    _showModalList(
      ...
      builder: (context, controller) {
        final filtered = _almacenes.where((a) => a.nombre?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? true).toList();
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) => ListTile(
            ...
            leading: Container(
              width: 44, height: 44,
              decoration: BoxDecoration(color: AppColors.secondaryBase.withOpacity(0.08), borderRadius: BorderRadius.circular(10)),
              child: Icon(_getWarehouseIcon(filtered[i].icono), color: AppColors.secondaryBase, size: 22),
            ),
            title: Text(filtered[i].nombre ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
            subtitle: Text('${filtered[i].registros} artículos'),
            onTap: () {
              setState(() { _selectedAlmacen = filtered[i]; _articulos = []; });
  ...
  void _showArticuloSelection() {
    _showModalList(
      ...
      builder: (context, controller) {
        final filtered = _articulos.where((a) => a.nombre?.toLowerCase().contains(_searchQuery.toLowerCase()) ?? true).toList();
        return ListView.builder(
          controller: controller,
          itemCount: filtered.length,
          itemBuilder: (ctx, i) {
            final tieneImagen = filtered[i].foto != null && filtered[i].foto!.isNotEmpty;
            return ListTile(
              contentPadding: const EdgeInsets.symmetric(horizontal: 20, vertical: 4),
              leading: Container(
                width: 44, height: 44,
                ...
                child: ClipRRect(
                  ...
                  child: tieneImagen
                    ? Image.network(filtered[i].foto!, fit: BoxFit.cover, errorBuilder: (_,__,___) => const Icon(Icons.inventory_2_outlined, color: AppColors.secondaryBase, size: 22))
                    : const Icon(Icons.inventory_2_outlined, color: AppColors.secondaryBase, size: 22),
                ),
              ),
              title: Text(filtered[i].nombre ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
              subtitle: Text(filtered[i].descripcion ?? 'Sin descripción', maxLines: 1, overflow: TextOverflow.ellipsis),
              onTap: () {
      ...
      builder: (context, controller) {
        return ListView.builder(
          ...
          itemBuilder: (ctx, i) => ListTile(
            ...
            leading: Container(
              width: 44, height: 44,
              decoration: BoxDecoration(color: AppColors.secondaryBase.withOpacity(0.05), borderRadius: BorderRadius.circular(10)),
              child: const Icon(Icons.assignment_outlined, color: AppColors.secondaryBase, size: 22),
            ),
            title: Text(_tipos[i].nombre ?? '', style: const TextStyle(fontWeight: FontWeight.bold)),
            onTap: () {
      ...
      builder: (ctx) => SafeArea(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 24),
            const Text('Estado de la Orden', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            ..._estados.map((estado) => ListTile(
              leading: Icon(Icons.circle, color: _getStatusColor(estado), size: 16),
              title: Text(estado, style: const TextStyle(fontWeight: FontWeight.bold)),
              trailing: _selectedEstado == estado ? const Icon(Icons.check_circle, color: AppColors.secondaryBase) : null,
              onTap: () {
  ...
  void _showModalList({required String title, required Widget Function(BuildContext, ScrollController) builder, bool showSearch = true}) {
    showModalBottomSheet(
      context: context,
